...
export function getBotResponse(userQuery: string): BotReply {
  ...
  if (q.includes("hustle") || q.includes("mania") || q.includes("selling")) {
    const event = EQUINOX_SUB_EVENTS.find((e) => e.id === "hustle-mania")!;
    return {
      text: `Here is **Hustle Mania** (Page 06 of the program). It tests live on-campus selling, negotiation, and guerrilla marketing!`,
      eventCard: event,
  ...
  if (q.includes("poly") || q.includes("monopoly") || q.includes("board")) {
    const event = EQUINOX_SUB_EVENTS.find((e) => e.id === "startup-poly")!;
    return {
  ...
  if (q.includes("ipl") || q.includes("auction") || q.includes("cricket") || q.includes("bidding")) {
    const event = EQUINOX_SUB_EVENTS.find((e) => e.id === "ipl-auction")!;
    return {
  ...
  if (q.includes("pitch") || q.includes("deck") || q.includes("investor") || q.includes("funding")) {
    const event = EQUINOX_SUB_EVENTS.find((e) => e.id === "pitch-deck")!;
    return {
  ...
  if (q.includes("spotlight") || q.includes("keynote") || q.includes("speaker") || q.includes("talk")) {
    const event = EQUINOX_SUB_EVENTS.find((e) => e.id === "spotlight")!;
    return {
  ...
  if (q.includes("crossroad") || q.includes("cross roads") || q.includes("case study") || q.includes("case")) {
    const event = EQUINOX_SUB_EVENTS.find((e) => e.id === "crossroads")!;
    return {
  ...
  if (q.includes("brand") || q.includes("battle") || q.includes("debate") || q.includes("rival")) {
    const event = EQUINOX_SUB_EVENTS.find((e) => e.id === "brand-battles")!;
    return {
  ...
  if (q.includes("internship") || q.includes("job") || q.includes("career") || q.includes("hire") || q.includes("drive")) {
    const event = EQUINOX_SUB_EVENTS.find((e) => e.id === "internship-drive")!;
    return {
  ...
  if (q.includes("expo") || q.includes("booth") || q.includes("stall") || q.includes("showcase")) {
    const event = EQUINOX_SUB_EVENTS.find((e) => e.id === "startup-expo")!;
    return {
  ...
  if (q.includes("e-cell") || q.includes("ecell") || q.includes("meet") || q.includes("conclave")) {
    const event = EQUINOX_SUB_EVENTS.find((e) => e.id === "e-cell-meet")!;
    return {
  ...
  if (q.includes("event") || q.includes("sub-event") || q.includes("competition") || q.includes("all")) {
    return {
      text: `The Equinox 2.0 features **10 official sub-events** from the program:\n\n**Page 05:**\n1. Spotlight\n2. Cross Roads\n3. Startup Expo\n4. Brand Battles\n5. IPL Auction\n\n**Page 06:**\n6. Hustle Mania\n7. Internship Drive\n8. Startup Poly\n9. E-Cell Meet\n10. Pitch Deck\n\nTap on any event below to preview its card and animation!`,
  ...
  return {
    text: `I am your **Equinox AI Assistant**, grounded directly in the official event program.\n\nAsk me about any of the **10 sub-events**, dates (**30 - 31 OCT**), MLRIT venue, or coordinators!`,
    suggestions: [
