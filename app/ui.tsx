...
export function Avatar({
  ...
  return (
    <div
      className={`grid place-items-center rounded-full bg-[#151515] border border-white/30 text-[#33FF67] font-bold ${className}`}
    >
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="min-w-0">
        ...
        <p className="truncate font-mono text-xs text-[#33FF67]">{role}</p>
      </div>
    </div>
  );
}
...
export function ContactCard({
  ...
  return (
    <div className="flex flex-col gap-4 rounded-2xl border-2 border-white/30 bg-white/10 p-6 backdrop-blur-xs transition hover:border-[#33FF67] hover:bg-white/15">
      <Person name={name} role={role} />
      <div className="data flex flex-col gap-1 text-sm">
        <a href={`mailto:${email}`} className="text-[#33FF67] font-medium hover:underline">
          {email}
        </a>
        {phone && (
          <a
            href={`tel:${phone.replace(/\s/g, "")}`}
            className="text-white/80 font-mono hover:text-[#33FF67]"
          >
            {phone}
          </a>
        )}
  ...
  return (
    ...
    >
      ...
      <ViewTransition name={`event-${event.slug}`} share="morph" default="none">
        <div className="relative aspect-[4/3] overflow-hidden rounded-2xl border-2 border-white bg-[#151515] p-4 flex items-center justify-center shadow-[4px_4px_0px_rgba(0,0,0,0.2)] transition duration-200 group-hover:bg-[#1e1e1e]">
          <SubEventBadge slug={event.slug} />
          <span className="label absolute top-3 left-3 rounded-full border border-white/40 bg-[#2A2A2A]/80 px-2.5 py-0.5 text-[10px] font-bold text-white uppercase backdrop-blur-md">
            {event.category}
          </span>
          <span className="absolute right-3 bottom-3 grid h-9 w-9 place-items-center rounded-full bg-white text-[#2A2A2A] opacity-0 transition duration-200 group-hover:opacity-100">
            <Arrow />
          </span>
        </div>
      </ViewTransition>
