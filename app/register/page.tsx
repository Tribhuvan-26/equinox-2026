...
export default function RegisterPage() {
  return (
    <PageTransition>
      <section className="brochure-grid min-h-screen bg-[#1B5FD6] text-white pt-40 pb-24">
        <div className="mx-auto max-w-[1400px] px-4 sm:px-8">
          ...
          <ol className="mt-16 grid gap-4 overflow-hidden sm:grid-cols-2 lg:grid-cols-4">
            {registration.steps.map((step, i) => (
              <li key={step.title} className="flex flex-col gap-3 rounded-2xl bg-[#0B2D6D]/85 border border-white/20 p-6 backdrop-blur-md">
                <span className="text-xs font-black uppercase tracking-wider text-[#F7CA50]">
          ...
          <h2 className="mt-20 heading text-3xl font-black text-[#F7CA50]">Passes</h2>
          <div className="mt-6 flex flex-col gap-6">
            {tickets.map((ticket) => (
              ...
              >
                <div className="flex flex-col gap-6 bg-[#0B2D6D]/95 p-8 text-white">
                  ...
                  <ul className="flex flex-col gap-3">
                    {ticket.perks.map((perk) => (
                      <li key={perk} className="border-l-2 border-[#F7CA50] pl-3 text-white/90">
                        {perk}
                      </li>
                    ))}
                  </ul>
                ...
                <div className="relative flex flex-col justify-between gap-10 bg-gradient-to-br from-[#FF4D79] to-[#E03260] p-8 text-white">
                  <div className="flex flex-col gap-4">
                    ...
                    <div>
                      <p className="data text-5xl font-black text-[#F7CA50]">{ticket.price}</p>
                      <p className="label mt-1 text-white/90 font-medium">{ticket.unit}</p>
                    </div>
                  </div>
                  <a
                    href={registration.formUrl}
                    className="press flex items-center justify-between rounded-xl bg-[#F7CA50] px-5 py-4 font-black text-[#0B2D6D] shadow-md transition-all hover:bg-[#ffe082] hover:scale-[1.02]"
                  >
                    Get this pass
                    <Arrow />
                  </a>
          ...
          <h2 className="mt-20 heading text-3xl font-black text-[#F7CA50]">Register for an event</h2>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {events.map((item) => (
              <Link
                ...
                className="flex items-center justify-between gap-4 rounded-2xl bg-[#0B2D6D]/85 border border-white/15 p-6 text-white transition-all hover:border-[#F7CA50] hover:bg-[#0B2D6D]"
              >
                <span>
                  <span className="block font-black text-lg text-white">{item.name}</span>
                  <span className="block text-sm text-[#F7CA50]">
          ...
          <h2 className="mt-20 heading text-3xl font-black text-[#F7CA50]">Stuck on registration?</h2>
